...
export async function getRandomSong(configs){ 
    let {type, explicit, languages, accessToken} = configs 

    let track = await doRequestGetRandomSong(type, accessToken)
    if((type === 'music' || type === '') && !explicit){
        if(track.explicit){
            let doLoop = true
            do{
                doLoop = false
                track = await doRequestGetRandomSong(type, accessToken)
                if(track.explicit) doLoop = true
            }while(doLoop)
        }
    }else if(type === 'podcast' && languages){
        if(languages.length){
            let doLoop = true
            do{
                if(track){
                    let {languages: trackLanguages} = track

                    for(let i = 0; i < trackLanguages.length; i++){
                        let trackLanguage = trackLanguages[i].substr(0, 2)

                        for(let j = 0; j < languages.length; j++){
                            let language = languages[j]
                            if(trackLanguage === language) doLoop = false

                        }
                    }
                }
                if(doLoop) track = await doRequestGetRandomSong(type, accessToken)
